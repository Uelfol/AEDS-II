...
void selecao_natural(FILE *arq, Lista *nome_arquivos_saida, int M, int nFunc, int *qtd_particoes){
    rewind(arq); //É utilizado para posicionar o cursor no inicio do arquivo
    ...
    if (reservar == NULL || v == NULL || salva_gravado == NULL){
        ...
        while (reg != nFunc) {
            //Faz a leitura dos M primeiros registros do arquivo e insere no vetor
             while (stop == 0) {
                fseek(arq, reg* tamanho_registro(), SEEK_SET);
                v[i] = le_funcionario(arq);
                i++;
            ...
            if (menor_memoria->cod > salva_gravado->cod || inicial == 0){
                ...
                if (aberto == 0){
                    ...
                    aberto = 1;
                    *qtd_particoes+=1; //Salvamos aqui a qtd_particicoes para usar o algoritmo de intercalação básico/árvore de vencedores
                ...
                *salva_gravado = *menor_memoria;
                ...
                //Para o caso de ter espaço no reservatório e o registro na memória ser menor que o último registro salvo
                if (count < M){
                    count = reservatorio(menor_memoria, count, reservar, M); //É enviado para o reservatório
                    reg++;
                }
                else{
                    //Reservatorio cheio
                    count = 0;
                    aberto = 0;
                    ordena(v, M); //Ordena o restante

                    //Escreve na partição o que se encontra na memória
                    for (int j = 0; j < M; j++){
                        fseek(p, r*tamanho_registro(), SEEK_SET);
                    ...
                    //Coloca o reservatório na memória
                    for (int j = 0; j < M; j++){
                    ...
                    fclose(reservar);
                    reservar = fopen("reservatorio.dat", "wb+"); //Faz a limpeza do reservatório
                    inicial = 0;
                    fclose(p);
                }
            }
            //Faz a leitura do próximo registro no arquivo
            if (inicial != 0 && reg < 20){
                fseek(arq, reg*tamanho_registro(), SEEK_SET);
                free(v[pos]);
    ...
    int c = 0;
    //Tras o que sobrou do reservatório para a memória
    rewind (reservar);
    ...
    ordena(v, M);
    //Faz a gravação o que estava no reservatório
    for (int j = 0; j < c; j++){
        fseek(p, r*tamanho_registro(), SEEK_SET);
        salva_funcionario(v[j], p);
        imprime_funcionario(v[j]);
...
void ordena (TFunc **v, int M){
    //Ordena os que já se encontram na memória para a partição
    int i = 0;
    for (int j = 1; j < M; j++) {
        TFunc *f = v[j];
        i = j - 1;
...
int reservatorio (TFunc *func, int contador, FILE *reservatorio, int M){
    //Se o reservatório não estiver cheio, salva o registro
    if (contador < M){
...
//É utilizado para ordenar as partições e serem intercaladas posteriormente
void organiza_p (Lista *nome_arq, int qtd, int nFunc){
    ...
    //Pega da partição para a memória, faz a ordenação e salva de novo na partição
    for (int i = 0; i < qtd; i++){
        nomes = nome_arq->nome;
        pt = fopen (nomes, "rb+");
...
//Faz a exibição das partições
void exibe (Lista *nome_arq, int qtd, int nFunc){
    FILE *pt;
    char *nomes;
    TFunc **v = calloc(sizeof(**v), nFunc);

    printf("\n\nPartições ordenadas com sucesso!");
