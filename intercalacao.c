...
void intercalacao_arvore_de_vencedores(int qtd_p, Lista *nomes_p){
    ...
    if (saida == NULL || list == NULL || salva_menor == NULL || v == NULL){
        ...
        //Aqui as partições são abertas e os primeiros registros são carregados para a memória
        ...
        vencedor(v, tam, saida, &k); //Obtemos quem é o primeiro vencedor
        ...
        while (fim < qtd_p){
            ...
            for (int i = 0; i < qtd_p; i++){
                ...
                if (fgetc(list[i].f) == EOF && list[i].fim_p == 0 && salva_menor->cod == v[aux]->cod){
                    ...
                    //Substituimos as ocorrências
                    for (int j = 0; j < tam; j++){
                        if (v[j]->cod == salva_menor->cod) v[j]->cod = 15000;
                    }
                    //Precisamos encontrar um novo vencedor
...
void vencedor (TFunc **func, int n, FILE *saida, int *c){
    ...
    for (int i = n-1; i>0; i--){
        ...
        if (i%2 != 0 && i == n-1){ //Um filho ou seja, posição na memória, ímpar
            pos = (i-1)/2;
            *func[pos] = *func[i];
        }
        else{
            if (i%2 == 0 && func[i]->cod < func[i-1]->cod){ //Caso o nó filho da direita seja menor que o da esquerda
                pos = (i-2)/2;
                *func[pos] = *func[i];
            }
            else if (i%2 == 0 && func[i-1]->cod < func[i]->cod){ //Nó filho da esquerda menor que o da direita
                pos = (i-2)/2;
                *func[pos] = *func[i-1];
            }
        }
